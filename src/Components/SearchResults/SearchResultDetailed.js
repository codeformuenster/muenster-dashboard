...
import './SearchResultDetailed.css'
...
/**
 * This component displays detailed information about one search result,
 * like the distance and approximate arrival times.
 */
export class SearchResultDetailed extends React.Component {
  /**
     * componentDidMount() is invoked immediately after a component
     * is mounted (inserted into the tree). Initialization that requires DOM nodes should go here.
  ...
  toHumanReadableDate(date) {
    const isoDate = this.isoDateStringToDate(date)
    if (isoDate.getHours() > 0) {
      return isoDate.toLocaleDateString('de-DE', {
  ...
  render() {
    ...
    return (
      <article key={result.id} className="notification detailedItem">
        <div className="media">
          ...
          <div className="media-content">
            <div className="content">
              <div className="pull-right closeBtn">
                <button
                  onClick={(e) => this.toggleSelection()}
                  className="delete"
                  aria-label="delete"
                />
              </div>
