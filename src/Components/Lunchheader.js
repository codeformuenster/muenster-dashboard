...
/**
 * This component encompasses the header which contains the logo
 * and the navigation bar. On small screens, the menu items
 * collapse to a hamburger icon.
 */
export class Lunchheader extends React.Component {
  constructor(props) {
    super(props)
    this.state = {
      district: '',
    ...
    // define a callback function that is called with the device's
    // position when it could be determined
  ...
  render() {
    ...
    if (district) {
      currentDistrict = <p className="control">Du bist hier: <span className="lunchtext">{district}</span></p>
    }
    return (
      <div>
